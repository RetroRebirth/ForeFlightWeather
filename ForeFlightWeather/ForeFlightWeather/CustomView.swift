...
public struct CustomView: View {
    ...
    func updateWeatherViewsWith(_ weather: Weather) {
        weatherViews = [WeatherView(text: weather.report.conditions.text,
                                    ...
                                    tempC: weather.report.conditions.tempC ?? -1.0,
                                    dewpointC: weather.report.conditions.dewpointC ?? -1.0,
                                    pressureHg: weather.report.conditions.pressureHg ?? -1.0,
                                    pressureHpa: weather.report.conditions.pressureHpa ?? -1.0,
        ...
        for forecast in weather.report.forecast.conditions {
            weatherViews.append(WeatherView(text: forecast.text,
                                            ...
                                            tempC: forecast.tempC ?? -1.0,
                                            dewpointC: forecast.dewpointC ?? -1.0,
                                            pressureHg: forecast.pressureHg ?? -1.0,
                                            pressureHpa: forecast.pressureHpa ?? -1.0,
